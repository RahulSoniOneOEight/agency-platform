...
Approved written architecture. Milestone F is merged (PR #23, merge commit `1d81ff107568c6f06ed4c7e3e7b0c2207d6e11f9`), so implementation is unblocked. The concrete F evidence shapes below are binding integration inputs for the reference-client proof.
...
Milestone F is merged and its final shapes have been inspected. The reference-client integration MUST consume these committed artifacts as supporting evidence rather than create parallel copies:

- `client-projects/reference-commerce/reference-e2e/report/reference-report.json`
  - current approval summary includes v1 and v2 with `review_round`, `review_state_hash`, and `source_commit_sha`
  - `qa`, `feedback`, `workflow.validator_outcomes`, assertions, fixture identity, and report identity are supporting evidence
- `client-projects/reference-commerce/reference-e2e/evidence/review-approval-evidence.json`
  - proves review, feedback, Approval v1, and QA promotion boundaries
- `client-projects/reference-commerce/reference-e2e/evidence/change-scenarios-evidence.json`
  - proves contract-impacting Approval v2 and implementation-only no-reapproval behavior
- `client-projects/reference-commerce/reference-e2e/evidence/resume-evidence.json`
  - proves workflow recovery and auditability

For the reference client, G treats the F machine report as regression/supporting evidence only. It does not become approval authority. Production authorization records reference approval version/hash/source identity explicitly, and production callers remain responsible for supplying the canonical current ApprovalSnapshot identity from the approval authority.
